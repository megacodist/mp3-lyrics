...
# Dependencies:
# 1. Python 3.10+
"""This module offers a list which is intrinsically kept sorted.
...
"""Holds the type of elements inside the SortedList object."""
...
class _DataComparerPair(object):
    ...
    def _RaiseTypeError(self, __op: str, __obj: Any, /) -> None:
        raise TypeError(f"'{__op}' not supported between instances of "
            f"'_DataComparerPair' and '{type(__obj).__name__}'")

    def __gt__(self, __obj: Any, /) -> bool:
        if not isinstance(__obj, _DataComparerPair):
            self._RaiseTypeError('>', __obj)
        return self.comparer > __obj
    
    def __lt__(self, __obj: Any, /) -> bool:
        if not isinstance(__obj, _DataComparerPair):
            self._RaiseTypeError('<', __obj)
        return self.comparer < __obj
    
    def __ge__(self, __obj: Any, /) -> bool:
        if not isinstance(__obj, _DataComparerPair):
            self._RaiseTypeError('>=', __obj)
        return self.comparer >= __obj
    
    def __le__(self, __obj: Any, /) -> bool:
        if not isinstance(__obj, _DataComparerPair):
            self._RaiseTypeError('<=', __obj)
        return self.comparer <= __obj
    ...
    def __ne__(self, __obj: Any, /) -> bool:
        if not isinstance(__obj, _DataComparerPair):
            self._RaiseTypeError('!=', __obj)
        return self.comparer != __obj


class SortedList(Sequence, Generic[_ElemType]):
    ...
    * This class is not thread safe.
    ...
    def __init__(
            ...
            *,
            ...
            key: Callable[[_ElemType], Any] | None = None
            ) -> None:
        '''Initializes a new instance of the class with two optional
        keyword-only arguments.
        
        The 'cp' parameter accepts a CollisionPolicy which
        ...
        callable must accepts an object of the type of elements of the soretd
        list and returns an object of arbitrary types which implements
        python comparison magic methods.
        This attribute can not be changed. To change
        this attribute, you must instantiate a new sorted list object.
        '''
        ...
        self._items: list[_ElemType] = []
    
    @property
    def items(self) -> list[_ElemType]:
    ...
    @items.setter
    def items(self, __list: list[_ElemType], /) -> None:
        del self._items
        self._items: list[_ElemType] = []
        self.merge(__list)
    ...
    def __getitem__(self, __idx: int, /) -> _ElemType:
        if self._key:
    ...
    # Implementing the iterator protocol (__iter__ and __next__)...
    def __iter__(self) -> Iterator[_ElemType]:
        # Settingand returning the iterator variable...
        self._iterIndex = 0
        return self
    
    def __next__(self) -> _ElemType:
        try:
    ...
    def count(self, __value: _ElemType, /) -> int:
